...
|------|--------|----------------|
| Statements | `data.sec.gov/api/xbrl/companyfacts/CIK<padded>.json` (SEC EDGAR XBRL) | **Navigate-to-JSON**: the tab navigates directly to the JSON URL, then `Runtime.evaluate` reads `document.body.innerText` and projects US-GAAP concepts to clean field names in-page. Top-level navigation bypasses CORS entirely (no `fetch`/CORS dance). Only the small projection crosses CDP — the multi-MB companyfacts object stays in the page. |
...
- **`Page.enable()` is called per tab** before navigation. `setLifecycleEventsEnabled` is *not* used here because we wait on a fixed settle timeout rather than `networkIdle` — SEC/Yahoo JSON endpoints return fast and synchronously, and the JSON body is available as soon as the document loads. A 2.5–5.5s settle covers the largest companyfacts files (~4 MB).
