...
| `browser-cdp open <url> [waitMs]` / `read <targetId>` / `eval <targetId> <js>` / `click <targetId> <x> <y>` / `pdf <targetId> <out.pdf>` / `close <targetId>` / `list` | The ONE-SHOT sibling (`sdk/browser-cdp`, Node built-ins only): no daemon, one page websocket per command over the browser's `/json` endpoints, plain stdout. For a five-command job (open a page, read it, click once) or a caller that cannot live with the REPL's single-bare-expression print rule. Multi-step, stateful work stays on the REPL. `pdf` prints the tab at print-service options (US Letter, zero margins, backgrounds, CSS `@page` honoured), which is what a `--print-to-pdf` launch flag does NOT do (default margins spill a one-page invoice onto two). |
