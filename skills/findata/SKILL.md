...
|------|--------|----------------|
| Statements | `data.sec.gov/api/xbrl/companyfacts/CIK<padded>.json` (SEC EDGAR XBRL) | **Navigate-to-JSON + readiness poll**: the tab navigates directly to the JSON URL, then a single `Runtime.evaluate` both checks readiness and projects US-GAAP concepts to clean field names in one IIFE. Top-level navigation bypasses CORS entirely (no `fetch`/CORS dance). Only the small projection crosses CDP — the multi-MB companyfacts object stays in the page and is parsed exactly once. |
| Ticker→CIK | `www.sec.gov/files/company_tickers.json` | Same navigate-to-JSON; parsed into a map cached on `globalThis.__secTickerMap` for the server's lifetime (one fetch per session, not per call). |
| Prices | `query1.finance.yahoo.com/v8/finance/chart/<ticker>?interval=&range=&period1=&period2=` | Navigate-to-JSON + readiness poll. Yahoo's chart endpoint returns clean OHLCV + `regularMarketPrice` as JSON — no DOM scraping, so it's robust against page-layout changes. Going through the browser sends correct headers/cookies and sidesteps the crumb/consent walls that block `curl` (which gets 403 on stockanalysis.com and a 301 on Yahoo's history page). |

**Why polling, not lifecycle events.** Chrome fires *neither* `Page.loadEventFired` *nor* `networkIdle` for `application/json` navigations (verified) — so the event-driven wait `gsearch`/`xsearch` use can't apply here. Instead `findata` navigates, then polls the in-page projection IIFE every ~80ms until it returns `{ready:true}`. The IIFE returns `{ready:false}` while `document.body.innerText` is empty/unparseable, and `{ready:true, …}` once the JSON is fully there — so the call returns the instant the body is ready, not after a fixed settle. It also bails fast when a non-JSON `content-type` arrives (an error page) instead of waiting out the timeout. Measured time-to-ready: ~250–500ms for Yahoo / the ticker map, ~1s for the largest companyfacts files (~4 MB).

Per call: `Target.createTarget(about:blank, background)` → `Target.attachToTarget` → per-call `sessionId` → navigate + readiness-poll → fire-and-forget `closeTab` in `finally`. Same pattern as `gsearch`/`xsearch` — safe for parallel use, no `activeSessionId` clobbering.

**Typical latency** (warm server): `price` ~0.25s, `prices` ~0.4s, statements ~1.1–1.4s (the companyfacts fetch+parse dominates; the first statement call per session adds ~0.5s for the one-time ticker-map fetch).
...
- **No lifecycle events for JSON pages.** Chrome emits neither `Page.loadEventFired` nor `networkIdle` for `application/json` navigations, so `findata` polls `document.body.innerText` until it parses (see *How it works*). On a non-JSON response (e.g. a 429 error page) it bails immediately off the `content-type` rather than waiting out the poll timeout.
